...
import give from '../assest/images/detail/give.jpg'
import collect from '../assest/images/detail/collect.jpg'
import collected from '../assest/images/detail/collected.png'
import comment from '../assest/images/detail/comment.jpg'
import nice from '../assest/images/detail/nice.jpg'
import niced from '../assest/images/detail/niced.jpg'
...
class Detail extends React.Component {
    ...
    render(){
        return (
            <div id="detail" style={{"position":"relative"}}>
                <div >
                <div className="head">
                    <img src={back} alt="" style={{"margin-left":"0.5rem"}} onClick={this.back.bind(this)} />
                    <img src={give} alt=""  style={{"margin-left":"2.3rem"}} onClick={this.give.bind(this)}/>
                    <img src={collect} alt="" id='img' style={{"margin-left":"0.3rem"}} onClick={this.collect.bind(this)}/>
                    <img src={comment} alt=""  style={{"margin-left":"0.3rem"}} onClick={this.go.bind(this,this.props.location.state.sid)}/><span>{this.state.n}</span>
                    <img src={nice}  alt="" onClick={this.zan.bind(this)} style={{"margin-left":"0.3rem"}} /><span>{this.state.like+this.state.nice}</span>
                </div>
                <div className="main clearfix"  style={{"position":"relative"}}>
                    <span style={{"position":"absolute","color":"#fff","left":"0.49rem","bottom":"0.81rem","font-size":"0.45rem"}}>{this.state.obj.title}</span>
                    <span style={{"position":"absolute","color":"#ccc","right":"0.63rem","bottom":"0.32rem","font-size":"0.3rem"}}>{this.state.obj.image_source}</span>
                    <img src={this.state.obj.images} width={"100%"} alt=""/>
                </div>

                    {/*<link rel="stylesheet" href={href} />*/}
                    <div dangerouslySetInnerHTML = {{ __html: this.state.obj.body }}/>


                </div>
                <div className="give" style={{"display":"none"}} id="give">
                    <div style={{"width":"100%","height":"30rem","background":"#333","position":"absolute","left":"0","top":"0","opacity":"0.4","z-index":"1"}} onClick={this.none.bind(this)}></div>
                    <div className="main" style={{"z-index":"111","width":"6.18rem","position":"absolute","left":"7%","top":"3.15rem","background":"#fff",}}>
                        <ul className="clearfix" style={{"padding":"0 0.27rem 0.65rem"}}>
                            <h3 style={{"font-size":"0.35rem","padding":"0.45rem 0 0 0.5rem"}}>分享</h3>
                            <li>
    ...
    componentDidMount(){
        ...
        var sum=0;
        var num=0;
        axios.get("/api/4/news/"+this.props.location.state.sid).then(res=>{
            console.log(res.data.css[0]);
            href=res.data.css[0];
            this.setState({
                obj:res.data,
            });
        ...
        axios.get("/api/4/story-extra/"+this.props.location.state.sid).then(res=>{
            this.setState({
        ...
        //    是否被收藏
        // console.log(localStorage.getItem(this.props.location.state.sid));
        if(localStorage.getItem(this.props.location.state.sid)) {
            this.setState({
                tag: true
            });
            document.getElementById('img').src = collected
        }
       /*如过数据是通过link标签引入的话，会有一个白色的框来放置你的内容，需要自行插入数据，铺设页面
        setTimeout(() => {
            document.getElementsByClassName('img-place-holder')[0].innerHTML =
                `<div class="header-tp" style='position:relative'>
                    <img src=${this.state.obj.image} alt="" style="height: 4.25rem; width: 100%;">
                    <p class='p' style="color:#fff;width: 6.5rem;position: absolute;top: 2.51rem;font-size: .44rem;left: .31rem;" >${this.state.obj.title}</p>
                    <span style='position: absolute;bottom: .22rem;right: .36rem;font-size: .33rem;color: #ccc;'>${this.state.obj.image_source}</span>
                    </div>`
        }, 1000)*/
    }
    give(){
    ...
    zan(event){
        if(this.state.flag){
            event.target.src=nice;
            this.setState({
                nice:0,
                flag:false
            })
        }else{
            event.target.src=niced;
            this.setState({
                nice:1,
                flag:true
            })
    ...
    collect(event){
        if(!this.state.tag){
            event.target.src=collected;
            localStorage.setItem(this.props.location.state.sid,JSON.stringify({
                title:name,
                image:src,
                id:this.props.location.state.sid
            }))
        }else{
            event.target.src=collect;
            localStorage.removeItem(this.props.location.state.sid)
        }
        this.setState({
            tag:!this.state.tag
        })
